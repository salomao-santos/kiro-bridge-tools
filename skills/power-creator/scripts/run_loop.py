...
Adapted from skill-creator/scripts/run_loop.py. Same train/test split logic;
underlying model calls now go through scripts.runtime.
"""
...
def run_loop(
    ...
    name, original_description, content = parse_power_md(power_path)
...
def main():
    ...
    name, _, _ = parse_power_md(power_path)

    if args.report != "none":
        if args.report == "auto":
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            live_report_path = Path(tempfile.gettempdir()) / f"power_description_report_{power_path.name}_{timestamp}.html"
        else:
