...
Este proyecto fue construido sobre **documentación oficial** y **ejemplos públicos** de los ecosistemas Claude Code (Anthropic) y Kiro (AWS). Entrega tres meta-herramientas que cubren el ciclo completo de migración entre **Claude Code Skills** y **Kiro Powers**.

| Herramienta | Dirección | Propósito |
|-------------|-----------|-----------|
| [`skills/skill-to-power/`](./skills/skill-to-power/) | Skill → Power | Convierte una Skill de Claude Code en una Power de Kiro |
...
Cualquiera que use otra herramienta de IA — **Claude Code, Antigravity, Cursor, Codex, GitHub Copilot** — y quiera migrar a **Kiro IDE** o **Kiro CLI**, o el camino inverso.

- **¿Viniendo de Claude Code a Kiro?** Usa `skill-to-power` (si ya tienes Skills) o `power-creator` (desde cero).
- **¿Quedándote en Claude Code?** Usa `skill-creator` para crear y mejorar Skills compatibles con Kiro.

## Estructura de una Skill (Claude Code)
...
| Triggering vía `available_skills` de Claude | Enrutamiento por descripción de Kiro | Mecanismo de activación |
